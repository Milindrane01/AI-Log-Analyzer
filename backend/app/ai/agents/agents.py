...
class InvestigatorAgent:
    ...
    def run(self, evidence: Evidence, scratch: dict) -> Step:
        ...
        if first is not None:
            ...
            hypothesis = "No timestamped ordering available; treating the highest-severity group as primary."
            top = min(evidence.groups, key=lambda g: _SEVERITY_RANK.get(g["severity"], 9), default=None)
            if top:
...
class VerifierAgent:
    ...
    def run(self, evidence: Evidence, scratch: dict) -> Step:
        ...
        supported = bool(first) and (not dated or dated[0]["group_id"] == first["group_id"])
        ...
        return Step("verifier", "verify", {"verified": verified, "confidence": confidence, "note": note})
