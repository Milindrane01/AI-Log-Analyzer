...
CATALOG: dict[str, CommandTemplate] = {
    ...
    for t in [
        # --- kubernetes ---
        CommandTemplate("k8s_get_pods", "kubernetes", "kubectl get pods -n {namespace}",
                        "List pods and their status in a namespace", params=(_NAMESPACE,)),
        CommandTemplate("k8s_describe_pod", "kubernetes",
                        "kubectl describe pod {pod} -n {namespace}",
                        "Show pod events, restarts, and scheduling issues",
                        params=(_POD, _NAMESPACE)),
        CommandTemplate("k8s_logs", "kubernetes",
                        "kubectl logs {pod} -n {namespace} --tail=200",
                        "Read recent logs from a pod", params=(_POD, _NAMESPACE)),
        CommandTemplate("k8s_top_pods", "kubernetes", "kubectl top pods -n {namespace}",
                        "Show CPU/memory usage per pod", params=(_NAMESPACE,)),
        # --- linux ---
        CommandTemplate("linux_systemctl_status", "linux", "systemctl status {service}",
                        "Check whether a service is running", params=(_SERVICE,)),
        CommandTemplate("linux_journal", "linux", "journalctl -u {service} --since '1 hour ago'",
                        "Read recent service logs", params=(_SERVICE,)),
        CommandTemplate("linux_port_check", "linux", "nc -zv {host} {port}",
                        "Test TCP reachability to a host:port", params=(_HOST, _PORT)),
        CommandTemplate("linux_pg_ready", "linux", "pg_isready -h {host} -p {port}",
                        "Check PostgreSQL accepts connections", params=(_HOST, _PORT)),
        CommandTemplate("linux_disk", "linux", "df -h",
                        "Show disk usage (full disks cause many failures)"),
        CommandTemplate("linux_mem", "linux", "free -m",
                        "Show memory usage (OOM diagnosis)"),
        # --- aws ---
        CommandTemplate("aws_logs_tail", "aws",
                        "aws logs tail {log_group} --since 1h",
                        "Tail a CloudWatch log group", params=(_LOG_GROUP,)),
        CommandTemplate("aws_rds_describe", "aws",
